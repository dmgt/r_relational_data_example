...
CanadianCodCatch <- cod_only %>%
  ...
  filter(areatype == "DFO" | areatype == "NAFO") %>%
  group_by(year) %>%
  summarize(CN_catch=sum(Total_Catch))

#Second attempt
CanadianCodCatch <- cod_only %>%
  semi_join(CanadianCodareas) %>%
  group_by(year) %>%
  summarize(CN_catch=sum(Total_Catch))  # add varible for na's?

#Third - suggestions from GSI - still have na's issue
CanadianCodCatch <- cod_only %>%
  filter(areaid %in% CanadianCodareas) %>%
  group_by(year) %>%
  summarize(CN_catch=sum(Total_Catch))  

p1 <- ggplot(CanadianCodCatch, aes(y = CN_catch, x = year)) +
      geom_line() +
      theme_bw()
